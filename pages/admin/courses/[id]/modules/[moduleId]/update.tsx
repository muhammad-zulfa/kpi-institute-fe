...
const UpdateModule = withAuth(() => {
  const router = useRouter();
  const { loading, put, get, ready } = useContext(ApiContext);
  ...
  const { data, refetch: getModuleById } = useQuery('getModuleById', async () => {
    const data = await get(`/modules/${router.query.moduleId}`);
  ...
  const { mutateAsync: createModules } = useMutation(async (payload: any) => {
    const data = await put(`/modules/${router.query.moduleId}`, payload);
  ...
  useEffect(() => {
    setHistories([
      ...
      {
        title: `Update Course Module ${(router.query.moduleId as string).substring(0,4)}`,
        path: `/admin/courses/${router.query.id}/modules/${router.query.moduleId}/update`,
      },
    ]);
  }, []);
...
UpdateModule.getLayout = (page: any) =>
