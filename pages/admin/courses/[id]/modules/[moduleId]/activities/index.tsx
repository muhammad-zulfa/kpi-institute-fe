...
const Activities = withAuth(() => {
    ...
    },
    {
      ...
      render: (item: any) => (
        <div className="flex space-x-6 justify-center py-1">
          <Link
            href={`/admin/courses/${router.query.id}/modules/${router.query.moduleId}/activities/${item.id}`}
          >
  ...
  const {
    ...
    "fetch-activities-" + router.query.id,
    async () => {
  ...
  const deleteModule = async (id: string) => {
    const res = await del(`/activities/${id}`);
    if (res) {
      getActivities();
    }
  };
  ...
  return (
    <>
      ...
      <button
        onClick={() => {
          router.push(`/admin/courses/${router.query.id}/modules/${router.query.moduleId}/activities/add`);
        }}
        type="button"
